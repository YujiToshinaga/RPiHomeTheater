...
void i2c_init(int master_id, uint8_t slave_addr)
{
    ...
    sil_wrw_mem((uint32_t *)(base + I2C_DIV), 2500);

    // スレーブアドレスを設定する
    sil_wrw_mem((uint32_t *)(base + I2C_A), slave_addr);
}
...
void i2c_write(int master_id, uint8_t* data, uint8_t len)
{
    ...
    // 転送可能になるまで待つ
    while ((sil_rew_mem((uint32_t *)(base + I2C_S)) & I2C_S_TA_BIT)
            == I2C_S_TA_BIT);

    // 書き込むデータ長を設定する
    sil_wrw_mem((uint32_t *)(base + I2C_DLEN), len);

    // 転送モードを書き込みにする
    tmp = sil_rew_mem((uint32_t *)(base + I2C_C));
    sil_wrw_mem((uint32_t *)(base + I2C_C), (tmp & ~I2C_C_READ_BIT));

    // ステータスをクリアにする
    sil_wrw_mem((uint32_t *)(base + I2C_S),
            I2C_S_CLKT_BIT | I2C_S_ERR_BIT | I2C_S_DONE_BIT);

    // FIFOをクリアにして転送を開始する
    tmp = sil_rew_mem((uint32_t *)(base + I2C_C));
    sil_wrw_mem((uint32_t *)(base + I2C_C),
            tmp | (I2C_C_ST_BIT | I2C_C_CLEAR_CLEAR));

    // FIFOレジスタにデータを書き込む
    for (i = 0; i < len; i++) {
        while ((sil_rew_mem((uint32_t *)(base + I2C_S)) & I2C_S_TXD_BIT)
                == 0x0);
        sil_wrw_mem((uint32_t *)(base + I2C_FIFO), data[i]);
    }

    // 転送完了するまで待つ
    while ((sil_rew_mem((uint32_t *)(base + I2C_S)) & I2C_S_DONE_BIT) == 0x0);

    // エラー処理をする
    tmp = sil_rew_mem((uint32_t *)(base + I2C_S));
    if (tmp & I2C_S_CLKT_BIT) {
        sil_wrw_mem((uint32_t *)(base + I2C_S), tmp | I2C_S_CLKT_BIT);
    }
    if (tmp & I2C_S_ERR_BIT) {
        sil_wrw_mem((uint32_t *)(base + I2C_S), tmp | I2C_S_ERR_BIT);
    }
}
